...
void draw_char(spi_device_handle_t spi, uint16_t x, uint16_t y, uint16_t unicode, uint16_t color) {
    uint8_t index;
...
void draw_text(spi_device_handle_t spi, uint16_t x, uint16_t y, const uint16_t *unicode_text, uint16_t color) {
    while (*unicode_text) {
        draw_char(spi, x, y, *unicode_text, color);
        x += 6; // Ширина символа + промежуток
        unicode_text++;
    }
}
